# Camunda IBM Exception To Change Risk Map

Exception-to-change risk map for Camunda incidents and IBM change-window exposure.

![ci](https://github.com/mizcausevic-dev/camunda-ibm-exception-to-change-risk-map/actions/workflows/ci.yml/badge.svg)

## What this ships

This repo is a static Kinetic Gain evidence surface, not a placeholder page. It packages Camunda, IBM signals into a board-readable operating view:

- Exposure: where workflow, platform, or communications risk can become visible.
- Savings: where duplicate effort, drift, or manual coordination can be reduced.
- Investment: which next action deserves funding, ownership, or escalation.
- Story: how the evidence should be explained to executives or investors.

## Live surface

- Product page: https://mizcausevic-dev.github.io/camunda-ibm-exception-to-change-risk-map/
- Repository: https://github.com/mizcausevic-dev/camunda-ibm-exception-to-change-risk-map
- Portfolio atlas: https://portfolio.kineticgain.com/

## Evidence lanes

- process timer drift
- batch handoff risk
- exception queues
- change-window exposure
- automation ROI
- SLA causality

## Related surfaces

- [Camunda IBM Mainframe Handoff Ledger](https://mizcausevic-dev.github.io/camunda-ibm-mainframe-handoff-ledger/)
- [Camunda IBM Process Modernization Board](https://mizcausevic-dev.github.io/camunda-ibm-process-modernization-board/)
- [Camunda IBM SLA Breach Causality Router](https://mizcausevic-dev.github.io/camunda-ibm-sla-breach-causality-router/)
- [Camunda IBM Workflow Automation ROI Pack](https://mizcausevic-dev.github.io/camunda-ibm-workflow-automation-roi-pack/)

## Local verification

```bash
npm test
```

The validation script checks the data payload, generated page, interactive evidence table, related links, and footer links.
